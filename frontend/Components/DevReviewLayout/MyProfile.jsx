  ...
  const handleProjectLike = async (event, projectId) => {
    ...
    setMyProjects((projects) => projects.map((project) => {
      ...
      return {
        ...project,
        isLiked: !isLiked,
        likesCount: Math.max(0, getProjectLikesCount(project) + (isLiked ? -1 : 1)),
      };
    }));
        ...
        {[
          ...
          return (
            ...
            >
              ...
              {tab.id === "saved" && !savedProjectsLoading && (
                <span className={`text-xs px-2 py-0.5 rounded-md ml-1 font-bold ${isActive ? "bg-accent/10 text-accent" : "bg-surface-2 text-muted"}`}>
            ...
            {activeTab === "saved" && (
              ...
              >
                ...
                {savedProjectsLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {savedProjects.map((project) => <SavedProjectCard key={project._id} project={project} onRemove={handleUnsaveProject} />)}
                  </div>
                )}
              </motion.div>
            )}
